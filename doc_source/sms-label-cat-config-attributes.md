...
| enum |  Yes  |  List of strings  |  Use this parameter to define options that workers can choose from for this label category attribute\. Workers can choose one value specified in `enum`\.   | 
...
The following is an example of a label category configuration file that includes label category attributes that you may use for a 3D point cloud labeling job\. See the quotas for these parameters in [Label and label category attribute quotas](#sms-point-cloud-label-cat-limits)\.
...
The following is an example of a video frame keypoint label category configuration file with label category attributes\.
