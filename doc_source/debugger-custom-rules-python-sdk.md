# Use [*smdebug* Python library](https://github.com/awslabs/sagemaker-debugger/) with the Amazon SageMaker Python SDK to Create a Custom Rule<a name="debugger-custom-rules-python-sdk"></a>
...
The following code sample shows how to configure a custom `ImproperActivation` rule using the open source[*smdebug* Python library](https://github.com/awslabs/sagemaker-debugger/) with the Amazon SageMaker Python SDK\. This example assumes that the custom rule you’ve written has path */rules/custom\_rules\.py*\. You do not need to specify a pre\-built Docker image when using the Amazon SageMaker Python SDK as the `Estimators` handle this task\. 
