...
| --- | --- | 
| num\_round | The number of rounds to run the training\. Valid values: integer Default value: \-  | 
| num\_class | Required if objective=*multi:softmax* or *multi:softprob*\. Valid values: integer Default value: \-  | 
...
| silent | 0 means print running messages, 1 means silent mode\. Valid values: 0/1 Default value: 0  | 
...
| max\_leaves | Maximum number of nodes to be added\. Relevant only if `grow_policy`=`lossguide`\. Valid values: integer Default value: 0  | 
| max\_bin | Maximum number of discrete bins to bucket continuous features\. Used only if `tree_method`=`hist`\.  Valid values: integer Default value: 256  | 
...
| one\_drop | When this flag is enabled, at least one tree is always dropped during the dropout\. Valid values: 0/1 Default value: 0  | 
