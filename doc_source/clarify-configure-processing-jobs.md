...
1. You need to provide an input dataset as tabular file\(s\) in CSV or JSONLines format\. The input dataset must include a label column for bias analysis\. The dataset should be prepared for machine learning with any pre\-processing needed, such as data cleaning or feature engineering, already completed\.

1. You need to provide a model artifact that supports either the CSV or JSONLines file format as one of its content type inputs\. For post\-training bias metrics and explainability, we use the dataset to make inferences with the model artifact\. Each row minus the label column must be ready to be used as payload for inferences\.

1. When creating processing jobs with the SageMaker container image:
   ...
   + The IAM user/role of the caller must have permissions for SageMaker APIs\. We recommend that you use the managed policy `“arn:aws:iam::aws:policy/AmazonSageMakerFullAccess”`\.

## Getting Started with SageMaker Clarify Container<a name="clarify-processing-job-configure-container"></a>
...
The image URIs for the containers are in the form:
...
The following table lists the addresses by region\.


**Docker images for pre\-training bias detection**  
...
The SageMaker Clarify XAI container can be used by a SageMaker processing job at several stages in the lifecycle of the machine learning workflow\. The SageMaker XAI container is used with your datasets and models to compute three types of analysis\.
+ pre\-training bias metrics
+ post\-training bias metrics
+ SHAP values for explainability

You can control which of these three analyses are computed when you configure the processing job\. For pre\-training bias metrics, you need to provide the dataset\. You can compute post\-training bias metrics and explainability after your model has been trained by providing the dataset and model name\. You must configure the necessary parameters in the form of a JSON configuration file and provide this as an input to the processing job\.
...
## Configure processing job container, input and output parameters<a name="clarify-processing-job-configure-parameters"></a>

The Processing Job requires that you specify two input parameters: a dataset file\(s\) with input name `"dataset"` as S3 object or prefix and an analysis configuration file with input name `"analysis_config"` as an S3 object\. The job also requires one output parameter: the output location as an S3 prefix\.
...
Using SageMaker Python SDK, create a `Processor` using the SageMaker XAI container image URI:

```
XAI_ANALYZER_IMAGE_URI = '678264136642.dkr.ecr.>region>.amazonaws.com/sagemaker-xai-analyzer:beta-0.1'
xai_analyzer = Processor(base_job_name='xai-analyzer',
                         image_uri=XAI_ANALYZER_IMAGE_URI,
...
Then, you must also provide the input and output parameters for the `Processor`\. The dataset input is optional if you are providing the `"dataset_uri"`\.
...
Parameters for the analysis configuration must be provided in a JSON file\. The filename or path must be provided in the `ProcessingInput` parameter named `“analysis_config"`\. Examples of analysis configuration files are provided below\. In this JSON configuration file, the following parameters are specified:
...
+ `"facet"`: \(Optional\) A list of features that are sensitive attributes, referred to as facets, to be used for bias metrics in the form of pairs to include:
...
+ `"methods"`: A list of methods and their parameters for the analyses and reports\. If any section is omitted then it is not computed\.
  ...
  + `"shap"`: \(Optional\) Section on SHAP value computation\.
    + `"baseline"`: A list of rows \(at least one\) or S3 object URI to be used as the baseline dataset \(a\.k\.a background dataset\) in the Kernel SHAP algorithm\. The format should be the same as the dataset format\. Each row should contain only the feature columns/values and omit the label column/values
...
Corresponding predictor configuration:
