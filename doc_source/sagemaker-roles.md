...
An Amazon SageMaker user can grant these permissions with an IAM role \(referred to as an execution role\)\. The user passes the role when making these API calls: [CreateNotebookInstance](API_CreateNotebookInstance.md), [CreateTrainingJob](API_CreateTrainingJob.md), and [CreateModel](API_CreateModel.md)\.
